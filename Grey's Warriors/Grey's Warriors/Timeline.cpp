	...
	for (int i = 0; i <= coler; ++i)
	{
		...
		while (temp != NULL) {
			...
			if (i + 1 == numTimel) 
			{
				...
				if (numTitles % 2 == 0) {
					color(11);  gotoxy(posx-2, 4);  cout << char(179) << " "; gotoxy(posx, 4); cout << temp->title << " "; color(7);
				}
				else {
					color(11);  gotoxy(posx-2, 6); cout << char(179) << " "; gotoxy(posx, 6); cout << temp->title << " "; color(7);
				}
				numTitles++;
			}
			temp = temp->next;
